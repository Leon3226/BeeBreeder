...
export class ApiariesSelectorComponent implements OnInit {
  ...
  constructor(private apiaryService: ApiaryService, private computersService: ComputersService, private modService: ModService, private fb: FormBuilder) { }
  ...
  submitAdd(){
    ...
    this.apiaryService.addApiary(newApiary).subscribe(x => {
      this.updateApiaries();
      this.backToList();
  ...
  updateComputersList(){
    this.computersService.getComputers().subscribe(x => {
      ...
      this.computersOptions.push({name: "Unoccupied", computers : x.filter(comp => comp.apiaryId == null)})
      this.computersOptions.push({name: "Occupied", computers : x.filter(comp => comp.apiaryId != null)})
    })
  }

  onSelectAddComputer(apiaryId : number, computerId : number){
    this.selectionListEnabled = false;
    this.apiaryService.attachComputer(apiaryId, computerId).subscribe(x => this.updateApiaries())
  }

  onSelectRemoveComputer(apiaryId : number, computerId : number){
    this.apiaryService.detachComputer(apiaryId, computerId).subscribe(x => this.updateApiaries())
  }
  ...
  submitEdit(){
    ...
    this.apiaryService.editApiary(editedApiary.id, editedApiary).subscribe(x => {
      this.updateApiaries();
      this.backToList();
